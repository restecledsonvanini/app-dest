...
const SHEET_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="3" width="20" height="18" rx="2" /><path d="M8 3v18M2 9h20M2 15h20" /></svg>`;
...
export class SheetCard extends HTMLElement {
    connectedCallback() {
        ...
        this.innerHTML = `<a href="${href}" target="_blank" rel="noopener noreferrer" class="sheet-card" title="${title}">${SHEET_SVG}<span>${label}</span></a>`;
    }
}
